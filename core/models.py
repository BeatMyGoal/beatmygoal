        ...
        if not errors:
            if amount == None:
                ...
                logEntry = LogEntry(log=log,
                                    ...
                                    comment=comment)
            logEntry.save()
            self.log.goal.checkWinner(participant)
...
class Goal(models.Model):
    ...
    @classmethod
    def create(self, title, description, creator, prize, private_setting, goal_type, ending_value, unit, ending_date, winner=None):
        errors = []
        goal = None
    ...
    def checkWinner(self, user):
        userTotal = self.log.getUserTotal(user)
        if userTotal >= self.ending_value:
            self.endGoal(user)

    def endGoal(self, winner):
        self.winners.add(*winner if isinstance(winner, list) else winner)

    def isEnded(self):
        return self.winners.all().exists()
